...
def main():
    ...
    positions1 = routefinder(wire1)

    positions2 = routefinder(wire2)

    for j in range(len(positions1)-1):
        for k in range(len(positions2)-1):
            ...
            #which wire is vertical?
            if w1x == positions1[j+1][0]:
                w1h = False
                #wire 1 is vertical
            else:
                w1h = True
            if w2x == positions2[k+1][0]:
                #wire 2 is vertical
                w2h = False
            else:
                w2h = True

            if (w1h == True and w2h == True) or (w1h == False and w2h == False):
                pass
            #cross with w1 vertical and 2 horizontal
            elif (w1h == False) and (w2h == True):


                if ((w2y > w1y) and (w2y < positions1[j+1][1])) or ((w2y < w1y) and (w2y > positions1[j+1][1])):
                    if ((w2x < w1x) and (w1x < positions2[k+1][0])) or ((w2x > w1x) and (w1x > positions2[k+1][0])):
                        intersections.append([w1x, w2y])
                        #print("found one! 1x 2y", w1x, w2y)
            else:
            #cross with w1 horizontal and 2 vertical
                if ((w1y > w2y) and (w1y < positions2[k+1][1])) or ((w1y < w2y) and (w1y > positions2[k+1][1])):
                    if (w1x < w2x) and (w2x < positions1[j+1][0]) or ((w1x > w2x) and (w2x > positions1[j+1][0])):

                        intersections.append([w2x, w1y])
                        #print("found one! 2x 1y", w2x, w1y)


    for x in intersections:
        distance = abs(int(x[0])) + abs(int(x[1]))
