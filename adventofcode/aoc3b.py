...
def main():
    ...
    positions1 = routefinder(wire1, steps)

    positions2 = routefinder(wire2, steps)

    for x in positions1.keys():
        if x in positions2.keys():
            intersections.append(positions1[x] + positions2[x])
...
def routefinder(wire, steps):
    ...
    for x in wire:
        ...
        while inc < num:
            ...
            inc += 1
            steps += 1
            if (str(posX), str(posY)) in pos:
                pass
            else:
                pos[str(posX),str(posY)] = steps
